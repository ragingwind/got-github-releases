...
module.exports = function (repo, opts) {
	...
	var cleaner = function (v) {
		return opts.clean ? semver.clean(v) : v;
	};
	...
	return ghGot('repos/' + repo + '/releases').then(function (res) {
		...
		res.body.forEach(function (re) {
			if (semver.cmp(re.tag_name, comparator, version)) {
				releases[cleaner(re.tag_name)] = re;
				if (opts.sort) {
					sortedIndex.push(cleaner(re.tag_name));
				}
			}
		});
		...
		if (sortedIndex.length > 0) {
			...
			releases.latest = releases[releases.sortedIndex[0]];
		}
